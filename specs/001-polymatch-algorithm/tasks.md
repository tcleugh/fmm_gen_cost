...
- [ ] T030 [P] [US2] In `test/polymatch_test.cpp`, add `TEST(POLYMATCHAppConfig, cli_parses_polygon_flags)`: argc/argv with `--polygons`, `--polygon_id_name`, `--polygon_cost_name`, `--access_points`, `--ap_node_id_name`, `--ap_polygon_id_name`, `--through_penalty_factor`, `--boundary_epsilon`; assert values land in correct config fields
- [ ] T031 [P] [US2] In `test/polymatch_test.cpp`, add `TEST(POLYMATCHAppConfig, xml_parses_polygon_blocks)`: XML config with `<polygon>` and `<access_point>` blocks parses into matching fields. **Deferred:** XML reader not yet wired into `POLYMATCHAppConfig::load_arg`.
...
- [ ] T033 [P] [US2] In `test/polymatch_test.cpp`, add `TEST(POLYMATCHApp, fallback_link_only_when_zero_valid_polygons)`: polygon layer present but all polygons invalid (skipped); assert fallback to link-only (FR-012)
...
- [ ] T089 [P] [US2] In `test/polymatch_test.cpp`, add `TEST(POLYMATCHAppConfig, cli_parity_with_weightmatch)`: parse `polymatch --help` and `weightmatch --help` outputs; assert every flag accepted by weightmatch is also accepted by polymatch (excluding any polygon-specific additions) (FR-011)
...
- [ ] T038 [US2] Implement `POLYMATCHAppConfig` XML parsing in `src/mm/polymatch/polymatch_app_config.cpp`: detect `--config` flag, parse XML via ptree, populate sub-configs including new `<polygon>` and `<access_point>` blocks per `contracts/polymatch-cli.md`
...
- [ ] T064 [US1] Implement polygon candidate search in `src/mm/polymatch/polymatch_algorithm.cpp`: query `PolygonLayer::polygons_containing(point)` and `polygons_within_radius` (FR-006). **Pending:** polygon candidates require extending the `Candidate` type to represent polygon snaps (currently `Candidate` is edge-only).
- [ ] T065 [US1] Implement polygon-aware `POLYMATCH::match_traj()` in `src/mm/polymatch/polymatch_algorithm.cpp`: time-tracked phases — link + polygon candidate search, hybrid TransitionGraph, Viterbi, build C_Path. **Currently:** matcher delegates to `WEIGHTMATCH` (link-only); polygon-aware path is the major outstanding US1 work.
- [ ] T066 [US1] Implement `POLYMATCH::update_tg()` and `update_layer()` extending weightmatch with same-polygon override, polygon-entry/egress cost, through-routing via `shortest_polylink_to_polylinks`, and layer-0/N-1 special case (R7).
- [ ] T067 [US1] Implement `POLYMATCH::build_cpath()`: walk Viterbi `TGOpath`, expand to hybrid `C_Path` with negative polygon IDs, track per-segment entry_ap / egress_ap / is_through / distance_inside (R12), populate `PolyMatchResult.polygon_segments` (FR-010, FR-016).
...
- [ ] T071 [P] [US3] In `test/polymatch_test.cpp`, add `TEST(PolyMMWriter, polygon_id_encoding_negative_in_cpath)`: assert polygons appear as negated IDs in `opath`/`cpath`, positive IDs in `polygon_ids`. **Deferred:** requires the polygon-aware `build_cpath` (T067) to produce negated IDs end-to-end.
...
- [ ] T075 [P] [US3] In `test/polymatch_test.cpp`, add `TEST(PolyMMWriter, thread_safe_concurrent_writes)`: spawn 2 threads each writing 1000 distinct `PolyMatchResult`s via the same `PolyMMWriter`; parse the resulting CSV and assert all 2000 rows present, no corruption, no interleaving inside a row (FR-017)
