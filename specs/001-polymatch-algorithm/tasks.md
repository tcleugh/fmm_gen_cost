...
- [X] T085 Manually executed the polymatch CLI end-to-end (link-only against `example/data/edges.shp` + `test/data/weightmatch/trips_basic.csv` and polygon mode against the fixtures `polymatch_test` materializes). Output rows confirmed to match `contracts/polymatch-output.md`: link-only emits `id;cpath;mgeom`, polygon mode adds `polygon_ids;entry_aps;egress_aps;is_through;polygon_distances` with the negated-ID encoding (`-7` in cpath, `7` in polygon_ids) and the literal `-` token where APs are absent (mid-polygon start/end). **Pending:** `mgeom` is currently emitted empty for hybrid paths — building the hybrid LineString from a mix of edge geometries and polygon entry/inside/egress points is left for a future iteration.
