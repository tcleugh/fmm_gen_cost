...
- [X] T053 [P] [US1] In `test/polymatch_test.cpp`, add `TEST(MatchTraj, through_routing_no_gps_inside)` (defensive): GPS configured so polygon may be a candidate but no observation falls inside; assert that *if* `is_through == true`, both APs are recorded. **Note:** the full through-routing cost path inside `shortest_polylink_to_polylinks` is still deferred (Dijkstra-level AP-context tracking); the defensive contract is verified here.
