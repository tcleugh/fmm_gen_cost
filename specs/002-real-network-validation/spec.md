...
- Polymatch's existing feature spec (`specs/001-polymatch-algorithm/spec.md`) is authoritative for matcher behavior. This feature only adds a validation harness on top; it does NOT change the matcher's algorithmic behavior. One narrow robustness fix landed alongside this feature: `AccessPointLayer::load` previously halted (`return false`) for any AP that was neither link-attached nor shared between ≥ 2 polygons. Polymatch spec FR-005 lists only three halt conditions (boundary mismatch, orphan polygon reference, contradictory shared-AP geometry) — the over-strict halt for "orphan single-polygon AP" was implementation drift, not a spec requirement. The loader now skips such APs with a warning instead. This change is necessary because OSM-derived real fixtures routinely contain such orphan APs, and the matcher's behavior on *usable* APs is unchanged.
